   ...
   var Staff = {
     ...
     datatable: function(deleted=0){
      Staff.staffTable=$('#staff').DataTable({
          "processing": true,
          "responsive":true,
          "serverSide": true,
          "searching":true,
         ...
         lengthMenu: [
          ...
          "ajax" : {
            url : Application.getPath()+'staff/datatable',
            method : "POST",
            data:{
              deleted:deleted
            }
     ...
     delete:function(){
      $(document).on("click", ".btn-delete", async function(event){
        ...
        Staff.confirmTask(Request,"delete",1);
      ...
      restore:function(){
        $(document).on("click", ".btn-restore", async function(event){
          ...
          Staff.confirmTask(Request,"restore",1);          
        });
        },
      edit:function(){
        $(document).on("click", ".btn-edit", async function(event){
          ...
          window.location.href=Application.getPath()+'staff/index?id='+id;
        });
      },
      detail:function(){
        $(document).on("click", ".btn-detail", async function(event){
          ...
          window.location.href=Application.getPath()+'staff/detail?id='+id;
        });
      },
      showAll:function(){
      ...
      deleteSelected:function(){
        $(document).on("click", "#btn-delete-selected", async function(event){
          event.preventDefault();
            ...
            if(idArray.length!==0){
            $('#alert').modal('show');
            Staff.confirmTask(Request,"delete",idArray.length);
            } 
        });
      },
      restoreSelected:function(){
        $(document).on("click", "#btn-restore-selected", async function(event){
            ...
            if(idArray.length!==0){
              $('#alert').modal('show');
              Staff.confirmTask(Request,"restore",idArray.length); 
            }
