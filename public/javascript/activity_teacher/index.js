...
var Teacher = {
  ...
  edit: async function () {
    ...
    if (id != undefined) {
      $("#submit").text("Update Teacher");
      let Response = await fetch(Teacher.url + '?id=' + id);
  ...
  validate: function () {
    $(document).on("submit", "#form-teacher", async function (event) {
      ...
      if (result["error"] == false) {
        ...
        if (id == undefined) {
          ...
          if (result.success == 1) {
            window.location.href = Application.getPath() + 'teacher/show';
          }
        } else {
          console.log("Update");
          //Posting value;
