...
class Student extends Model 
{
    ...
    public  function datatable($deleted)
    {
        try {
            ...
            $query = '';
            $output = array();
            $query .= "SELECT * FROM tblstudent WHERE deleted=$deleted ";
            ...
            $output = array(
              ...
              "recordsFiltered" => self::getTotalStudent($deleted),
              "data" => $data,
              "totalShowAll"=>self::getTotalStudent(0),
              "totalShowTrash"=>self::getTotalStudent(1),
          );
  ...
  public static function getTotalStudent($deleted=0)
  {
    $db = static::getDB();
    $statement = $db->prepare("SELECT * FROM tblstudent WHERE deleted=$deleted " );
    $statement->execute();
  ...
  //   $db = static::getDB();
  //   $statement = $db->prepare("UPDATE tblstudent SET deleted=0 WHERE deleted=$deleted " );
  //   $statement->execute();
  //   return $statement->rowCount();
  // }
